...
class Usuario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('username, password, apellido, nombre, email', 'required'),
			array('username, password, apellido, nombre, avatar, email', 'length', 'max'=>128),
			array('username, apellido', 'length', 'max'=>128),
			array('username, nombre', 'length', 'max'=>128),
			array('estado', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, username, password, estado, avatar, email', 'safe', 'on'=>'search'),
		);
	}
